...
$requests=$pdo->query("SELECT lr.*,bk.title,bk.author,bk.available_copies,CONCAT(s.first_name,' ',s.last_name) AS student_name,s.student_code,CONCAT(t.first_name,' ',t.last_name) AS teacher_name,t.teacher_code FROM library_requests lr JOIN library_books bk ON lr.book_id=bk.id LEFT JOIN students s ON lr.student_id=s.id LEFT JOIN teachers t ON lr.teacher_id=t.id WHERE lr.status='Pending' ORDER BY lr.requested_at ASC")->fetchAll();
